...
//*****************************************************************************
//
//  This is the revised version of SdfData unit.
//  Revised by Talat Dogan, January 2006.
...
// TBaseTextDataSet
  ...
  protected
    ...
    function RecordFilter(RecBuf: Pointer; RecNo: Integer): Boolean;
    function BufToStore(Buffer: PChar): String; virtual;
    function StoreToBuf(Source: String): String; virtual;
    function CheckString(sField:String; idx:Integer; const KeyValues: Variant;
...
uses
  Windows, Forms, DBConsts;

//-----------------------------------------------------------------------------
// TFixedFormatDataSet
//-----------------------------------------------------------------------------
constructor TFixedFormatDataSet.Create(Owner : TComponent);
begin
 FFileMustExist  := TRUE;
...
begin
  ...
  begin
    ...
    if len > Maxlen then
      Maxlen := len;
    FData.Objects[i] := Pointer(i+1);   // Fabricate Bookmarks
  end;
  if (Maxlen = 0) then
    Maxlen := MAXSTRLEN;
...
function TFixedFormatDataSet.RecordFilter(RecBuf: Pointer; RecNo: Integer): Boolean;
var
  Accept: Boolean;
...
begin                          // Returns true if accepted in the filter
  ...
  PRecInfo(FFilterBuffer + FRecInfoOfs).RecordNumber := RecNo;
  Accept := TRUE;
  try
...
function TFixedFormatDataSet.Locate(const KeyFields: string;
...
begin
  Result := False;
  FoundStr:=false;  //   Added by TDogan
  ActualRec := RecNo;

  if (FData.Count = 0) or (Trim(KeyFields) = '')then
    exit;

  LstSearch:= TStringList.Create;
  try
    LstSearch.CommaText := KeyFields;
    Self.First;

    while not Eof do     // search all records
    begin
      FoundStr := False;

      for i := 0 to LstSearch.Count -1 do  // matches all values of the inquiry
      begin
        FoundStr := CheckString(LstSearch[i], i, KeyValues, Options);
    ...
    // not found
    //   Below line disabled by TDogan January 05, 2006
    //  SetCurrentRecord(ActualRec);

    //   Below line Added by TDogan January 05, 2006
    if  FoundStr then SetCurrentRecord(ActualRec) else SetRecNo(ActualRec);
  finally
    LstSearch.Free;
  end;
end;

function TFixedFormatDataSet.CheckString(sField: String; idx: Integer;
  const KeyValues: Variant; Options: TLocateOptions): Boolean;
var
  sSearch : String;
  sCompare: String;
...
begin
  ...
  try
    ...
    sSearch := KeyValues[idx];
  end;

  sSearch := Trim(sSearch);
  sCompare := Trim(FieldByName(sField).AsString);

  len := Length(sSearch);
  p := Length(sCompare);
  if (p < len) or ((p > len) and not (loPartialKey in options)) then
    exit;

  Up := loCaseInsensitive in Options;

  for p := 1 to len do
    if (not Up and (sCompare[p] <> sSearch[p])) or
         (Up and (UpCase(sCompare[p]) <> UpCase(sSearch[p]))) then
      exit;
  ...
  begin
    ...
    begin
      ...
      if BufEnd > RecBuf then
        BufEnd := RecBuf;
      FillChar(BufEnd[0], Field.Size + RecBuf - BufEnd, Ord(' '));
      p := StrLen(Buffer);
      if p > Field.Size then
        p := Field.Size;
      Move(Buffer^, RecBuf[0], p);
      ActiveBuffer[RecordSize] := #0;
    end;
  end
...
procedure TFixedFormatDataSet.InternalAddRecord(Buffer: Pointer; Append: Boolean);
begin
  FSaveChanges := TRUE;
  Inc(FLastBookmark);
  if Append then
    InternalLast;
  if (FCurRec >=0) then
    FData.InsertObject(FCurRec, BufToStore(Buffer), Pointer(FLastBookmark))
  else
    FData.AddObject(BufToStore(Buffer), Pointer(FLastBookmark));
end;

procedure TFixedFormatDataSet.InternalGotoBookmark(Bookmark: Pointer);
var
  Index: Integer;
begin
...
// TSdfDataSet
//-----------------------------------------------------------------------------
constructor TSdfDataSet.Create(Owner: TComponent);
begin
  inherited Create(Owner);
  FDelimiter := ',';
...
begin
  ...
  for i := 0 to FieldDefs.Count - 1 do
   begin
    ...
    while Boolean(pStrEnd[0]) and (pStrEnd[0] in [#1..' ']) do
      Inc(pStrEnd);

    if not Boolean(pStrEnd[0]) then
     break;
    ...
    if (pStr[0] = '"') then
     begin
      repeat
        Inc(pStrEnd);
      until not Boolean(pStrEnd[0]) or
      ...
      while Boolean(pStrEnd[0]) and (pStrEnd[0] <> Delimiter) do
        Inc(pStrEnd);
